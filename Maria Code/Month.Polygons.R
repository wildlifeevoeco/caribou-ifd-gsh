libs <- c('sqldf','data.table','plyr','ggplot2','adehabitatHR',
          'gridExtra', 'sp','readr', 'dismo', 'dplyr')

lapply(libs, require, character.only = TRUE)


avg_daily_NNdist <- fread("Maria Code/avg_daily_NNdist.csv")
avg_season_NNdist <- fread("Maria Code/avg_season_NNdist.csv")
raw_all_NNdist <- fread("Maria Code/raw_all_NNdist.csv")
...
x<-unique(raw_all_NNdist2$CalvingDate)

library(tidyr)
calvingdata = data.frame(x)
calvingdates<- separate(calvingdata, "x", c("Year", "Month", "Day"), sep = "-")

summary(calvingdates)
##earliest calving date = 05_22
##latest calving date = 06_18
#lastest calving date +4 weeks = 06_18 + 0_28 = 07_16

y<-unique(raw_all_NNdist2$FIX_DATE)
fixdates = data.frame(y)
alldates<- separate(fixdates, "y", c("Year", "Month", "Day"), sep = "-")
...
NNnew$ANIMAL_ID <- as.factor(NNnew$ANIMAL_ID)

NNnew$lnArea <- log(NNnew$Area) 

library(lme4)

lm <- lmer(lnArea ~ MeanNN + CalvingGround + (1|Year) + (1|ANIMAL_ID) , data = NNnew)
summary(lm)
r.squaredGLMM(lm)
library(lmerTest)
