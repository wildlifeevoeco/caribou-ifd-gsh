...
library(Matrix)
library(MuMIn)
avg_season <- read_csv("~/Downloads/Honours Readings/Statistics /data/avg_season_NNdist.csv")
View(avg_season)
...
#splitdata1<-split(avg_season,avg_season$Year)
#splitdata1
...
m1 <- rpt(NNdistkm~ (1|Year)+(1|ID), data = avg_season, grname = c("ID"))
summary(m1)
plot(m1)
...
modswitch1<-rpt(CalvingGround2 ~ (1|Year) + (1|ID), data = avg_season,datatype = "Binary", 
             grname = c("ID"))
...
#as a function of year and ID
modcore1<- rpt(Core ~ (1|Year) + (1|ID), data = ondata, datatype = 'Binary',
              grname = c("ID"))
...
ggplot(data=subset(data1, !is.na(Switch)), aes(x = Switch, y = Dist.Off)) +
  geom_boxplot(outlier.colour = NA, notch = TRUE, notchwidth = 0.5, lwd = 2.5 ) +
